...
// Try to load build-time snapshot (generated by backend/scripts/fetch-all-data.js)
let SNAPSHOT_DATA: any = null;
try {
  // eslint-disable-next-line @typescript-eslint/no-require-imports
  SNAPSHOT_DATA = require('./data/snapshot.json');
} catch {
  SNAPSHOT_DATA = null;
}

// Map snapshot to static data format
const SNAPSHOT_DASHBOARD = SNAPSHOT_DATA?.dashboard || STATIC_DASHBOARD;
const SNAPSHOT_STORES = SNAPSHOT_DATA?.stores || STATIC_STORES;
const SNAPSHOT_VISITS = SNAPSHOT_DATA?.visits || STATIC_VISITS;
const SNAPSHOT_USERS = SNAPSHOT_DATA?.users || STATIC_USERS;
const SNAPSHOT_PRODUCT_REPORT = SNAPSHOT_DATA?.product_report || STATIC_PRODUCT_REPORT;
const SNAPSHOT_SF_REPORT = SNAPSHOT_DATA?.sf_report || STATIC_SF_REPORT;
const SNAPSHOT_SF_TREND = SNAPSHOT_DATA?.sf_trend || STATIC_SF_TREND;
const SNAPSHOT_BIZ_REPORT = SNAPSHOT_DATA?.biz_report || STATIC_BIZ_REPORT;

const API_URL = import.meta.env.VITE_API_URL || 'http://localhost:3001';
const STATIC_MODE = (import.meta.env.VITE_STATIC_MODE || 'true') === 'true';
const HAS_SNAPSHOT = SNAPSHOT_DATA !== null;
  ...
  const loadFromCacheOrFetch = async () => {
    ...
    if (STATIC_MODE) {
      ...
      setDbStatus({
        ...
        timestamp: SNAPSHOT_DATA?.generated_at || new Date().toISOString(),
      });
